...
- **Body Points** → `reference/copywriting-structures.md` (Value Loop section). **For criteria-list body points (N things to check / N rules of X) load** `reference/state-cost-application-rule-framework.md` — STATE/COST/APPLICATION/RULE per criterion + universal two-kinds-of-X opener + analogy test. **For single-concept body points load** `reference/coaching-feedback.md` 2026-05-24 entry — Foundation/Theory/Example/Application. **For Lauren scripts specifically, ALSO load** `reference/callaway-script-framework.md` (Callaway's 5-step empirical pattern + Value Loop + 2-1-3-4 ordering) AND `reference/body-point-packaging-method.md` (12-step packaging sequence, Callaway×Kyle conflict resolution, banned phrases, ICP adaptations for non-creator coaches, ship checklist). These two files are load-bearing for every Lauren body point.
...
4. **Don't give the mechanism away in the hook** ("Don't Give It Away", Benji). The hook states the *problem* or a *contrarian claim* and withholds the *answer* — the body pays it off. A hook line that IS the mechanism (however true and sharp) closes the loop: the viewer thinks "oh, I get it" and leaves. If you must name the mechanism, hide it behind an unexplained named container (a new tool / method / branded name). Authority + open curiosity are the two levers that hold the viewer past the hook — and authority is what earns the reveal in the body its trust.

Banned in intros: listing things the viewer might have tried; origin story before the hook; generic "in this video I'll teach you…" openers; **explaining what the video is actually about in the hook line (giving the mechanism away)**.

Full mechanics → `reference/copywriting-structures.md`.

**Before drafting any intro, also open `reference/intro-style-bank.md`** — a bank of 8 verified patterns (Dan Martell + Ed Lawrence + Benji's A/B teardown) tagged with pattern shape (Compressed Structure 1 / Paradigm-Shift Hook / Reframe + Delayed-Credibility / Question + Data Flex / Hyperbole + Immediate Dive / Structure 1 + Extended Analogy / **"Don't Give It Away" withhold-the-mechanism rubric**). Pick the matching pattern, read the exemplar, draft to match its beat sequence in Lauren's voice — then run every hook line through the Don't-Give-It-Away A/B test.
...
- [`reference/my-story-how-i-made-x.md`](reference/my-story-how-i-made-x.md) — **the "My Story / How I Made X" storytelling video type** (a DIFFERENT scripting logic from the tactical/tutorial scripts). Joseph Campbell **Hero's Journey** backbone + an 11-beat fill-in-the-blank origin-story template (hook/trailer → beginnings → inflection/last-straw → first attempts → going deeper → first results → epiphany → rise+ego → the fall/earthquake → stoicism turnaround → open-loop close) + core principles: continuity/no-jumps · identification · include ego+vulnerability · **end on an OPEN loop, never closed** · purpose = brand & loyal audience, not the sale. Use for the founder's-journey / origin video, usually the first in a channel sequence. Source: YT Alchemy; raw transcript in [`reference/source-transcript-my-story-how-i-made-x.md`](reference/source-transcript-my-story-how-i-made-x.md).
- [`reference/body-point-modules.md`](reference/body-point-modules.md) — **modular script composition system.** Body points = reusable structural SHAPES (numeric-walkthrough, contrast-pair, mechanism-reveal, N-component-enumeration, etc.) studied from top creators, cherry-picked and mixed to compose new scripts, filled with the user's own content in the user's own voice. NEVER verbatim. Shape taxonomy + slot specs + mix-and-match rules + IP boundary. Working library lives in the project's `template-bank/`. Read at Step 4 (sequence body shapes) + Step 7c (write each body point from its module).
